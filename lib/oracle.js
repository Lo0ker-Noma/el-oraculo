...
async function geminiJSON(prompt, { model, key, timeout = 90000, intentos = 3 }) {
  let res, ultimoError = "";
  for (let i = 0; i < intentos; i++) {
    ...
    // 429 = cuota por minuto del free tier: esperamos y reintentamos
    if (res.status === 429 && i < intentos - 1) {
      const espera = 20000 * (i + 1);
      console.log(`[oracle] Gemini 429, reintento en ${espera / 1000}s...`);
