 ...
 * Grok es el resolvedor principal: investiga con NUESTRAS herramientas (xAI
 * deprecó su Live Search) usando creditos de pago, sin limite diario. Gemini
 * (free, 20/dia) y Claude quedan de respaldo.
 */
 ...
 *
 * Muchas apuestas son asimetricas: "¿Boca gana la Sudamericana?" se puede dar
 * por FALSA en cuanto Boca queda eliminado (en cualquier momento), pero solo se
 * puede dar por VERDADERA tras la final. Esta funcion pregunta al agente si el
 * desenlace YA es irreversible; si no lo es, la apuesta sigue viva hasta su
 * fecha de resolucion.
 *
 * Devuelve {resoluble: false} o {resoluble: true, outcome, confianza, ...}.
 * Es deliberadamente conservadora: ante la minima duda, no adelanta nada.
 */
export async function comprobarDesenlaceAnticipado(market) {
  const key = process.env.GEMINI_API_KEY;
  if (!key) return { resoluble: false };
  const model = process.env.GEMINI_MODEL || "gemini-3.5-flash";
  try {
    const { evidencias, fuentes } = await recogerEvidencia(market.question, { model, key });
    const r = await geminiJSON(
      `Eres EL ORACULO, juez de una apuesta con dinero real en juego.\n\n` +
        `APUESTA: "${market.question}"\n` +
        (market.description ? `Contexto: ${market.description}\n` : "") +
        `Fecha actual: ${new Date().toISOString()}\n` +
        `Fecha limite de resolucion: ${new Date(market.resolves_at * 1000).toISOString()}\n\n` +
        `EVIDENCIA DE TUS HERRAMIENTAS:\n${evidencias.join("\n") || "(ninguna)"}\n\n` +
        `PREGUNTA: ¿el desenlace de esta apuesta YA es DEFINITIVO e IRREVERSIBLE, ` +
        `aunque no haya llegado la fecha limite?\n\n` +
        `Ejemplo: en "¿Boca gana la Copa Sudamericana?", si Boca ya fue ELIMINADO, ` +
        `el resultado es NO de forma irreversible y se puede resolver ya. Pero si Boca ` +
        `sigue en competencia, NO se puede adelantar nada aunque vaya perdiendo.\n\n` +
        `REGLAS ESTRICTAS:\n` +
        `1. Responde resoluble=true SOLO si el desenlace es imposible de revertir.\n` +
        `2. Ir ganando o perdiendo, ser favorito, o "casi seguro" NO cuentan: eso es resoluble=false.\n` +
        `3. Si te falta evidencia clara y actual, resoluble=false.\n` +
        `4. Ante CUALQUIER duda, resoluble=false. Es preferible esperar a la fecha limite ` +
        `que repartir mal el dinero de la gente.\n\n` +
        `SOLO JSON: {"resoluble":true|false,"outcome":"si"|"no"|null,` +
        `"razonamiento":"maximo 2 frases","fuentes":["url"]}`,
      { model, key }
    );
    if (!r?.resoluble || !["si", "no"].includes(r.outcome)) return { resoluble: false };
    return {
      resoluble: true,
      outcome: r.outcome,
      confianza: "alta",
      razonamiento: `[Resolución anticipada: el desenlace ya era irreversible] ${String(r.razonamiento || "").slice(0, 500)}`,
    ...
    console.warn(`[oracle] comprobacion anticipada fallo: ${e.message}`);
    return { resoluble: false };
  }
}
...
// --- Grok (xAI) — chat completions con NUESTRAS herramientas ----------------
// xAI deprecó su Live Search (410). En su lugar, Grok elige entre nuestras
// herramientas (lib/tools.js: CoinGecko, clima, Wikipedia...), nosotros las
// ejecutamos y Grok dicta el veredicto citando fuentes. Usa creditos de pago
// (sin el limite de 20/dia del free tier de Gemini).

const GROK_URL = "https://api.x.ai/v1/chat/completions";

async function grokJSON(system, user, { model, apiKey, timeout = 60000 }) {
...
async function resolveWithGrok(market) {
  ...
  try {
    // 1) que consultar: heuristica si es evidente, si no decide Grok
    let llamadas = planHeuristico(market.question);
    if (llamadas) {
      console.log(`[oraculo] (grok) plan heuristico -> ${llamadas.map((c) => c.herramienta).join(", ")}`);
    } else {
      const catalogo = toolsDisponibles()
        .map((t) => `- ${t.name}: ${t.descripcion} params: ${JSON.stringify(t.params)}`)
        .join("\n");
      const plan = await grokJSON(
        "Eres EL ORACULO. Eliges herramientas para verificar una apuesta con datos reales.",
        `${marketPrompt(market)}\n\nHerramientas disponibles:\n${catalogo}\n\n` +
          `Elige hasta 3 llamadas que te permitan verificar la apuesta. ` +
          `Responde SOLO JSON: {"llamadas":[{"herramienta":"nombre","params":{}}]}`,
        { model, apiKey }
      );
      llamadas = plan.llamadas || [];
    ...
    for (const c of llamadas.slice(0, 3)) {
      ...
      evidencias.push(`[${c.herramienta}] ${r.dato}`);
      fuentes.push(...r.fuentes);
      console.log(`[oraculo] (grok) herramienta ${c.herramienta} -> ${r.ok ? "ok" : "fallo"}`);
    }

    // 3) veredicto con la evidencia delante
    const verdict = await grokJSON(
      SYSTEM,
      `${marketPrompt(market)}\n\n` +
        `EVIDENCIA RECOGIDA POR TUS HERRAMIENTAS (${new Date().toISOString()}):\n` +
        (evidencias.length ? evidencias.join("\n") : "(ninguna: no pudiste verificar nada)") +
