...
const NWC_TIMEOUT_MS = Number(process.env.NWC_TIMEOUT_MS || 8000);
// Tras un timeout dejamos de intentar NWC un rato, pero SOLO para emitir
// facturas (ahi tenemos LNURL de recambio). Consultar pagos y repartir premios
// no tienen alternativa, asi que se intentan siempre.
const CAIDA_MS = 15_000;
 ...
 *  `critico`: operaciones sin alternativa (consultar pago, pagar premio) que se
 *  intentan SIEMPRE, aunque la wallet este marcada como caida. */
async function nwc(etiqueta, fn, { critico = false } = {}) {
  if (!critico && !nwcDisponible()) return null;
  try {
    ...
    // el "circuito abierto" solo afecta a lo que tiene recambio (facturas)
    if (!critico) salud.nwcCaidoHasta = Date.now() + CAIDA_MS;
    console.warn(`[wallet] NWC no responde (${etiqueta}: ${e.message}).`);
    return null;
  }
}
...
export async function isPaid(payment_hash) {
  ...
  const res = await nwc("lookup_invoice", (c) => c.lookupInvoice({ payment_hash }), { critico: true });
  if (!res) return false;
...
export async function ingresosLiquidados(limit = 50) {
  if (DEMO) return [];
  const r = await nwc("list_transactions", (c) => c.listTransactions({ limit }), { critico: true });
  if (!r) return [];
...
export async function payToAddress(lnaddress, sats, comment) {
  ...
  const r = await nwc("pay_invoice", (c) => c.payInvoice({ invoice: invoice.paymentRequest }), { critico: true });
